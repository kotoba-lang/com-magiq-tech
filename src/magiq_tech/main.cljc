(ns magiq_tech.main
  "Kotodama WASM entrypoint for the magiq_tech clean-room actor (L4) — Clojure port.
  ...
  facts (`magiq_tech.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "magiq_tech")
(def tier "L4")
...
(def entity-specs
  [{:entity "KeySession" :plural "keysessions" :id-prefix "magiqtec_key"
    :fields [:aliceNodeId :bobNodeId :protocol :status] :required [:protocol :status]
    :coerce {}                       :refs {}}
   {:entity "Node"       :plural "nodes"       :id-prefix "magiqtec_nod"
    :fields [:name :location :role]            :required [:name :location]
    :coerce {}                       :refs {}}
   {:entity "Channel"    :plural "channels"    :id-prefix "magiqtec_cha"
    :fields [:sessionId :medium :lossDb]       :required [:medium :lossDb]
    :coerce {:lossDb :float}         :refs {:sessionId "KeySession"}}
   {:entity "Key"        :plural "keys"        :id-prefix "magiqtec_key"
    :fields [:sessionId :lengthBits :qber :contentRef] :required [:lengthBits :qber]
    :coerce {:lengthBits :int :qber :float} :refs {:sessionId "KeySession"}}
   {:entity "Protocol"   :plural "protocols"   :id-prefix "magiqtec_pro"
    :fields [:name :family]                    :required [:name :family]
    :coerce {}                       :refs {}}
   {:entity "Alarm"      :plural "alarms"      :id-prefix "magiqtec_ala"
...
(defn emit-facts
  "EAVT facts for one record: {\"magiq_tech.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "magiq_tech-compat" :tier tier :entities entities} 200])
...
(defn start! [] :magiq_tech-compat/ready)
